...
As a user
  ...
  Background:
    Given a logged in user with username "owner"
  ...
  Scenario: Add a member to a todo list
    And another user exists with username "member"
  ...
  Scenario: Owner removes a member from a todo list
    ...
    And another user exists with username "friend"
  ...
  Scenario: Non-owner cannot delete a todo list
    ...
    And another user exists with username "intruder"
    And the current user is switched to "intruder"
